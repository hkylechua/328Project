...
target_intensity = input("What level of intensity would you like to focus on during your workout session: low, moderate, or vigorous?\n" )
sys.stdout.flush()
...
def predict(window):
    ...
    label= classifier.predict(np.array([feature_vector[1]]))
...
app.layout = html.Div(
	[
		html.Div(
			children=[
				html.H1(children="Group 3's Final Project: HeartFit", className="header-title"),
				html.P(children=["HeartFit helps to track the intensity of your workout session. Messages will appear below to let you know if you are within your target workout intensity.", html.Br(),"Streamed from Sensor Logger: tszheichoi.com/sensorlogger", html.Br(),
...
# 	)
	...
	if len(sensor_data) > window_size:
		intensity = predict(np.asarray(sensor_data[-window_size:]))

    #######################################################################################################
	message_div = None
# 	target_div = html.Div(
# 		html.P(children="Target Workout Intensity: {}".format(target_intensity), style={"color": "red", "font-weight": "bold"}),
# 		className="textcard",
# 	)
	activity_div = html.Div(
		html.P(children=["Target Workout Intensity: {}".format(target_intensity), html.Br(), "Current Workout Intensity: {}".format(intensity)], style={"color": "red", "font-weight": "bold"}),
		className="textcard",
	)
    
	if intensity == target_intensity:
		message_div = html.Div(
			html.P(children="Great job! You are within target workout intensity", style={"color": "red", "font-weight": "bold"}),
			className="textcard",
		)
	else:
		if target_intensity == "low" and (intensity == "moderate" or intensity == "vigorous"):
			message_div = html.Div(
				html.P(children="You are above your target workout intensity. Try slowing down.", style={"color": "red", "font-weight": "bold"}),
			className="textcard",
			)
		if target_intensity == "vigorous" and (intensity == "moderate" or intensity == "low"):
			message_div = html.Div(
				html.P(children="You are below your target workout intensity. Try picking up the pace!", style={"color": "red", "font-weight": "bold"}),
			className="textcard",
			)
		if target_intensity == "moderate" and intensity == "low":
			message_div = html.Div(
				html.P(children="You are below your target workout intensity. Try picking up the pace!", style={"color": "red", "font-weight": "bold"}),
			className="textcard",
			)
		if target_intensity == "moderate" and intensity == "vigorous":
			message_div = html.Div(
				html.P(children="You are above your target workout intensity. Try slowing down.", style={"color": "red", "font-weight": "bold"}),
			className="textcard",
			)           
	...
	return html.Div(graphs), activity_div, message_div
	# return text
